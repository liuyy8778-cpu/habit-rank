...
const LIB = [
  { id:'k1',  type:'habit', dom:'螢幕自律', domColor:'indigo',  where:'home',     times:{home:'22:30',school:'21:40'}, label:'睡前準時交機',        desc:'螢幕會一路把你拖到深夜、偷走隔天的精神；主動交機、離開房間，守住「能放下」這塊最重要的肌肉。', coin:20, xp:15, honestyEligible:true, unlockRank:0, icon:'i-moon' },
  { id:'k2',  type:'habit', dom:'螢幕自律', domColor:'indigo',  where:'home',     times:{home:'22:00',school:'21:20'}, label:'準時結束今天的螢幕',    desc:'「再看一下、再一下」，時間就一直在漏；到點自己收手，守住「是我決定停」而不是被關掉。',            coin:15, xp:12, honestyEligible:true, unlockRank:0, icon:'i-hour' },
  { id:'sc3', type:'task',  dom:'螢幕自律', domColor:'indigo',  where:'anywhere', label:'今天完全沒偷超時',      sub:'偷一點超時會慢慢變常態，守住一整天的約定',   coin:8, xp:6, unlockRank:0, icon:'i-check' },
  { id:'sc4', type:'task',  dom:'螢幕自律', domColor:'indigo',  where:'home',     label:'起床後 30 分不碰手機',  sub:'一抓手機整個早上就被吸走，守住清醒的開場',     coin:8, xp:6, unlockRank:1, icon:'i-bolt' },
  { id:'sc5', type:'task',  dom:'螢幕自律', domColor:'indigo',  where:'home',     label:'用完手機主動放回充電座', sub:'留在手邊就會再被點開，歸位守住剛剛的收手',        coin:5, xp:4, unlockRank:3, icon:'i-shield' },
  { id:'sc6', type:'task',  dom:'螢幕自律', domColor:'indigo',  where:'anywhere', label:'吃飯時不看螢幕',        sub:'螢幕一開就吃不進飯也聽不見人，守住這頓飯的相處',     coin:5, xp:4, unlockRank:3, icon:'i-heart' },
  { id:'sl1', type:'habit', dom:'作息自律', domColor:'sky',     where:'anywhere', times:{home:'22:45',school:'22:00'}, label:'準時上床睡覺',        desc:'晚睡會一天天把生理時鐘往後推；到點就上床，守住身體記得住的入睡節奏。',              coin:15, xp:12, honestyEligible:true, unlockRank:2, icon:'i-moon' },
  { id:'sl2', type:'task',  dom:'作息自律', domColor:'sky',     where:'anywhere', label:'鬧鐘響第一次就起床',    sub:'每按一次貪睡就多漏掉一段早晨，第一次就起，守住完整的開始',   coin:10, xp:8, unlockRank:1, icon:'i-hour' },
  { id:'sl3', type:'task',  dom:'作息自律', domColor:'sky',     where:'home',     label:'睡前準備好明天的東西',  sub:'早上臨時找東西會手忙腳亂，先備好，守住明天出門的從容',   coin:5, xp:4, unlockRank:3, icon:'i-brief' },
  { id:'ld1', type:'task',  dom:'學習自律', domColor:'teal',    where:'home',     label:'回家先寫完作業再玩',    sub:'先玩就會愈拖愈晚，先寫完，守住之後真正放鬆的時間',             coin:10, xp:8, unlockRank:0, icon:'i-brief' },
  { id:'ld2', type:'task',  dom:'學習自律', domColor:'teal',    where:'home',     label:'專注讀書 30 分',        sub:'手機一滑剛進入的專注就散了，守住這 30 分的效率',       coin:10, xp:8, unlockRank:2, icon:'i-chart' },
  { id:'ld3', type:'task',  dom:'學習自律', domColor:'teal',    where:'anywhere', label:'主動多做一點',          sub:'會的東西不練就會慢慢生疏，多做一點，守住已經到手的實力',         coin:6, xp:5, unlockRank:3, icon:'i-spark' },
  { id:'bd1', type:'task',  dom:'身體自律', domColor:'amber',   where:'anywhere', label:'離線運動 30 分',        sub:'整天坐著盯螢幕身體會愈來愈鈍，動一動，守住你的體力和精神', coin:8, xp:6, unlockRank:0, icon:'i-bolt' },
  { id:'bd2', type:'task',  dom:'身體自律', domColor:'amber',   where:'anywhere', label:'喝足夠的水',            sub:'不知不覺就一整天沒喝水，喝滿，守住不昏沉的腦袋',         coin:4, xp:3, unlockRank:3, icon:'i-target' },
  { id:'bd3', type:'task',  dom:'身體自律', domColor:'amber',   where:'anywhere', label:'好好吃完正餐',          sub:'挑掉不吃體力就一點點虧空，好好吃完，守住撐一整天的燃料',             coin:4, xp:3, unlockRank:2, icon:'i-heart' },
  { id:'rp1', type:'task',  dom:'責任自律', domColor:'teal',    where:'home',     label:'完成一件家事',          sub:'沒人做家裡的事就會一直積著，做一件，守住這個家是一起顧的',           coin:8, xp:6, unlockRank:1, icon:'i-check' },
  { id:'rp2', type:'task',  dom:'責任自律', domColor:'teal',    where:'home',     label:'自己整理房間 / 書桌',   sub:'亂一點點就會愈積愈亂，隨手整理，守住找得到東西的空間',           coin:5, xp:4, unlockRank:3, icon:'i-gear' },
  { id:'rp3', type:'task',  dom:'責任自律', domColor:'teal',    where:'home',     label:'東西用完歸位',          sub:'隨手一放下次就到處找，用完歸位，守住不必再找的省事',           coin:4, xp:3, unlockRank:4, icon:'i-target' },
  { id:'em1', type:'task',  dom:'情緒自律', domColor:'magenta', where:'anywhere', label:'情緒踩煞車',            sub:'氣頭上一句話就收不回來，先暫停深呼吸，守住不會後悔的自己', coin:8, xp:6, unlockRank:0, icon:'i-heart' },
  { id:'em2', type:'task',  dom:'情緒自律', domColor:'magenta', where:'anywhere', label:'說到做到',              sub:'答應了卻沒做信任就一點點漏掉，做到，守住別人對你的話算數',       coin:8, xp:6, unlockRank:2, icon:'i-shield' },
];
